...
MultiBot:SetScript("OnEvent", function()
	...
	if(event == "CHAT_MSG_SYSTEM") then
		...
		if(MultiBot.isInside(arg1, "player already logged in")) then
			...
			if(MultiBot.isMember(tName)) then
				tButton.waitFor = "CO"
				SendChatMessage(MultiBot.doReplace(MultiBot.info.combat, "NAME", tName), "WHISPER", nil, tName)
	...
	if(event == "CHAT_MSG_WHISPER") then
		...
		if(MultiBot.isInside(arg1, "Hello", "你好")) then
			tButton.waitFor = "CO"
			SendChatMessage(MultiBot.doReplace(MultiBot.info.combat, "NAME", arg2), "WHISPER", nil, 2)
		...
		if(tButton.waitFor == "CO" and MultiBot.isInside(arg1, "Strategies: ")) then
			...
			SendChatMessage(MultiBot.doReplace(MultiBot.info.normal, "NAME", arg2), "WHISPER", nil, arg2)
			SendChatMessage("nc ?", "WHISPER", nil, arg2)
			return
		end
