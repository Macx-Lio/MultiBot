MultiBot.newCharacter = function(pParent, pName, pClass, pX, pY, pSize)
	...
	frame.setRadio = function(pButton)
		...
		for k, v in pairs(frame.radio[pButton.radio]) do
			if(k ~= pButton.config[2]) then v.setState(false) end
		end
	...
	frame.setLink = function(pButton)
		...
		for k, v in pairs(frame.link[pButton.link]) do
			if(k ~= pButton.config[2]) then v.setState(pButton.state) end
		end
